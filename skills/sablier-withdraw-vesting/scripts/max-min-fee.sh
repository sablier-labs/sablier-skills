#!/usr/bin/env bash
#
# max-min-fee.sh — Batch-query Sablier `calculateMinFeeWei` via Multicall3
# and return the MAX across all input streams as a decimal wei string.
# One RPC round trip per chunk regardless of stream count; replacement for
# the fragile per-stream `cast call | awk | sort | tail -1` loop the older
# runbook used.
#
# Why MAX (not SUM): `withdrawMultiple` `delegatecall`s into `_withdraw`
# for each stream and Solidity preserves `msg.value` across delegatecall,
# so a single `msg.value` covers every iteration's fee gate. The contract
# receives the fee exactly once per outer call. See the runbook's
# "Why MAX, not SUM" rationale and `lockup/src/SablierLockup.sol`.
#
# Use only for v3.0+ groups. v1.x is non-payable; v2.x is payable but the
# function `calculateMinFeeWei` is not deployed.
#
...
#
# Output (stdout):
#   A single decimal string: max(calculateMinFeeWei) across the input.
#   Empty input prints "0".
#
# Usage:
#   max-min-fee.sh --rpc-url <url> [--chain-id <id>] [--multicall <addr>]
#                  [--chunk <n>] [--input <file>]
#
# Example:
#   echo "$GROUP" | jq '.streams' \
#     | scripts/max-min-fee.sh --rpc-url https://mainnet.base.org --chain-id 8453
#
# Notes:
...
#     batch. That is deliberate — every v3.0+ Lockup exposes
#     `calculateMinFeeWei`, so a revert signals a real problem (wrong
#     chain, wrong contract, indexer bug) worth surfacing loudly.
#   - Requires: cast, jq.
...
while [ "$i" -lt "$COUNT" ]; do
  ...
  for j in $(seq "$i" $((END - 1))); do
    DATA=$(cast calldata "calculateMinFeeWei(uint256)" "${TOKEN_IDS[$j]}")
  ...
  while IFS= read -r HEX; do
    DEC=$(cast to-dec "$HEX")
    # Decimal string compare avoids 256-bit overflow in shell math.
    if [ "${#DEC}" -gt "${#MAX_FEE}" ] || \
       { [ "${#DEC}" -eq "${#MAX_FEE}" ] && [ "$DEC" \> "$MAX_FEE" ]; }; then
      MAX_FEE="$DEC"
    fi
  done <<< "$CHUNK_HEXES"
